<template>
  ...
  // Generar 1 números aleatorios únicos dentro del rango de IDs de Pokémon para la PC
  while (pokemonIdsPC.value.length < 1) {
  ...
  const selectedPokemon5 = ref([]);
  
  // Obtener los IDs de los Pokémon seleccionados por el jugador desde el localStorage
  const storedSelectedPokemon = localStorage.getItem('selectedPokemon5');
  if (storedSelectedPokemon) {
    selectedPokemon5.value = JSON.parse(storedSelectedPokemon);
  }
  ...
  const selectPokemon = (pokemonId) => {
    ...
    showGoButton.value = selectedPokemon5.value.length > 0;
  
  };
  
  // Función para verificar si un Pokémon está seleccionado
  const isSelected = (pokemonId) => {
    return selectedPokemon5.value.includes(pokemonId);
  };
  ...
  const comparePokemonStats = async () => {
    ...
    try {
        ...
        // Determinar el ganador de la batalla de Pokémon
        let battleResult = '';
        if (winningIndex !== -1) {
            const winningPokemonId = pokemonIdsInBattle[winningIndex];
            if (selectedPokemon5.value.includes(winningPokemonId)) {
                // El jugador ganó la batalla de Pokémon
                console.log("¡Ganaste!");
                battleResult = 'win';
                playerPoints.value += 2;
            } else {
                // La PC ganó la batalla de Pokémon
                console.log("¡La PC ganó!");
                battleResult = 'lose';
                pcPoints.value += 2;
            }
        } else {
            // Hubo un empate en la batalla de Pokémon
        ...
        // Guardar el resultado de la batalla en localStorage
  ...
  const filteredSelectedPokemon = computed(() => {
    return selectedPokemon5.value.filter(id => !battlePokemon.value.includes(id));
  });
