<template>
  ...
  // Generar 4 números aleatorios únicos dentro del rango de IDs de Pokémon para la PC
  while (pokemonIdsPC.value.length < 4) {
  ...
  const storedSelectedPokemon = localStorage.getItem('selectedPokemon2');
  if (storedSelectedPokemon) {
    selectedPokemon2.value = JSON.parse(storedSelectedPokemon);
  }
  ...
  const selectPokemon = (pokemonId) => {
    ...
    showGoButton.value = selectedPokemon2.value.length > 0;
  
  };
  
  // Función para verificar si un Pokémon está seleccionado
  const isSelected = (pokemonId) => {
    return selectedPokemon2.value.includes(pokemonId);
  };
...
const updateAndSaveSelectedPokemon = () => {
  const updatedSelectedPokemon = selectedPokemon2.value.filter(id => !battlePokemon.value.includes(id));
  localStorage.setItem('selectedPokemon3', JSON.stringify(updatedSelectedPokemon));
};
  ...
  const comparePokemonStats = async () => {
    ...
    try {
      ...
      if (winningIndex !== -1) {
        ...
        if (selectedPokemon2.value.includes(winningPokemonId)) {
          // El jugador ganó
          console.log("¡Ganaste!");
  ...
  const filteredSelectedPokemon = computed(() => {
    return selectedPokemon2.value.filter(id => !battlePokemon.value.includes(id));
  });
